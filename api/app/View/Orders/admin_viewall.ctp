...
<table class="table-striped table-bordered table-condensed table-hover">
    <tr>
        ...
        <th>Total Qrcode amount</th>  
        <th>Total Table Reservation amount</th>   
    </tr>
     <?php  foreach ($alldata['Order'] as $order): ?>
        <tr>
                <td><?php echo h($order['name']); ?></td>
                <td><?php echo h($order['dtotal']); ?></td>           
                <td><?php echo h($order['ptotal']); ?></td> 
                <td><?php echo h($order['qrtotal']); ?></td> 
...
<table class="table-striped table-bordered table-condensed table-hover">
    <tr>
        ...
        <th>Pickup amount</th>
        <th>Qrcode amount</th>  
        <th>Table Reservation amount</th>   
    </tr>
     <?php  foreach ($alldata['Order'] as $order): ?>
        <tr>
                <td><?php echo h($order['name']); ?></td>
                <td><?php echo h(($order['dtotal']*$rdata['Restaurant']['commission_del'])/100); ?></td>           
                <td><?php echo h(($order['ptotal']*$rdata['Restaurant']['commission_pick'])/100); ?></td> 
                <td><?php echo h(($order['qrtotal']*$rdata['Restaurant']['commission_qr'])/100); ?></td> 
